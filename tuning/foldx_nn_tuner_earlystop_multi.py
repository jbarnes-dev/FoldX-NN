...
def build_model(hp):
    ...
    for i in range(hp.Int('num_middle_layers',min_value=0,max_value=3,step=1)):
        j=i+1
        model.add(
                layers.Dense(
                    units = hp.Int(f"units_{j}", min_value=16, max_value=512, step=16),
                    activation = hp.Choice(f"activation_{j}", activation_functions),
                    )
                )
